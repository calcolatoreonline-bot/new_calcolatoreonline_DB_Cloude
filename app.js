...
async function generaGraficiProgressione(pdf) {
  try {
    ...
    // stesso identico stile già usato per le altre mini-dashboard del PDF
    const disegnaSfondoCardSagoma = (x, y, w, h) => {
      const raggioCard = 3, insetCard = 1.5;
      pdf.setGState(new window.jspdf.GState({ opacity: 0.5 }));
      pdf.setFillColor(248, 250, 252);
      pdf.roundedRect(x - insetCard, y - insetCard, w + insetCard * 2, h + insetCard * 2, raggioCard, raggioCard, 'F');
      pdf.setGState(new window.jspdf.GState({ opacity: 1 }));
      pdf.setDrawColor(226, 232, 240);
      pdf.setLineWidth(0.25);
      pdf.roundedRect(x - insetCard, y - insetCard, w + insetCard * 2, h + insetCard * 2, raggioCard, raggioCard, 'S');
    };
