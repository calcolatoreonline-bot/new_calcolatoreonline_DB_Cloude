...
async function generaGraficiProgressione(pdf) {
  try {
    ...
    // affiancate in una fila uniforme. Con una sola serie (es. Somma Pliche, Coscia) la
    // card NON si allarga per tutta la larghezza del grafico — resterebbe troppo vuota
    // ai lati — ma si restringe a una dimensione proporzionata al contenuto e si centra.
    // Sfondo grigio chiarissimo neutro (non violetto: si confondeva col grafico sopra) +
    // bordo grigio chiaro + angoli arrotondati, stesso principio della card della tabella
    // "Riepilogo Complessivo" ma con un colore che si stacca bene da entrambi i lati.
    const aggiuntaMiniDashboardBox = (x, y, width, dati) => {
      ...
      const gapColonne = 3;
      const larghezzaCard = width;
      const xCard = x + (width - larghezzaCard) / 2;
      const larghezzaColonna = (larghezzaCard - gapColonne * (numColonne - 1)) / numColonne;

      const raggioCard = 3;
      const insetCard = 1.5;
      pdf.setGState(new window.jspdf.GState({ opacity: 0.5 }));
      pdf.setFillColor(248, 250, 252);
      pdf.roundedRect(xCard - insetCard, y - insetCard, larghezzaCard + insetCard * 2, miniDashHeight + insetCard * 2, raggioCard, raggioCard, 'F');
      pdf.setGState(new window.jspdf.GState({ opacity: 1 }));
      pdf.setDrawColor(226, 232, 240);
      pdf.setLineWidth(0.25);
      pdf.roundedRect(xCard - insetCard, y - insetCard, larghezzaCard + insetCard * 2, miniDashHeight + insetCard * 2, raggioCard, raggioCard, 'S');

      dati.serie.forEach((serieSingola, i) => {
        const xColonna = xCard + i * (larghezzaColonna + gapColonne);
...
function disegnaMiniDashboardIG(ctx, x, y, larghezza, serieList, unit, decimals, colore) {
  ...
  // Con una sola serie (es. Peso, Somma Pliche, Coscia) la card NON si allarga per
  // tutta la larghezza del grafico — resterebbe troppo vuota ai lati — ma si restringe
  // a una dimensione proporzionata al contenuto e si centra. Sfondo grigio chiarissimo
  // neutro (non violetto: si confondeva col grafico sopra) + bordo grigio chiaro +
  // angoli arrotondati.
  const gap = 14;
  const larghezzaCard = larghezza;
  const xCard = x + (larghezza - larghezzaCard) / 2;
  const colW = (larghezzaCard - gap * (n - 1)) / n;

  ctx.save();
  ctx.globalAlpha = 0.5;
  ctx.fillStyle = 'rgb(248, 250, 252)';
  disegnaRettangoloArrotondatoIG(ctx, xCard, y, larghezzaCard, altezzaBlocco, 12);
  ctx.fill();
  ctx.restore();
  ...
  serieValide.forEach((s, i) => {
    ...
    ctx.beginPath();
    ctx.fillStyle = colore;
    ctx.arc(colX + padX + 4, y + 22, 4, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.fillText(s.label.toUpperCase(), colX + padX + 14, y + 22);
