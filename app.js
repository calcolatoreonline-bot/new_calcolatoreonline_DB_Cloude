...
// Larghezza di riferimento "da desktop" per catturare i grafici: calcolata dalle
// stesse regole CSS del sito (.dashboard-container max-width 1280px - padding 20px
// per lato - gap 28px tra le 2 colonne, diviso 2). Su schermi stretti (telefono) il
// sito passa a 1 colonna per la visualizzazione normale, e Chart.js ridisegna il
// grafico con proporzioni molto diverse — questa costante serve a forzare SEMPRE la
// stessa forma "da desktop" un istante prima di ogni cattura (PDF e Instagram),
// indipendentemente dal dispositivo con cui si genera l'esportazione.
const LARGHEZZA_DESKTOP_CATTURA_CHART = 606;
...
// larghezza "da desktop" e ripristinando tutto subito dopo — stessa identica logica
...
function catturaCanvasChartConLarghezzaFissa(canvasEl) {
  ...
  const cardEl = canvasEl.closest('.chart-card') || canvasEl.parentElement;
  const chartInstance = window.Chart ? Chart.getChart(canvasEl.id) : null;
  const larghezzaOriginaleCard = cardEl ? cardEl.style.width : null;

  if (cardEl) {
    cardEl.style.width = LARGHEZZA_DESKTOP_CATTURA_CHART + 'px';
  ...
  if (cardEl) {
    cardEl.style.width = larghezzaOriginaleCard || '';
    if (chartInstance) chartInstance.resize();
  }
...
async function catturaCardInstagram(elementId, scale = 3) {
  ...
  if (!el) return null;
  const cardEl = el.closest('.chart-card') || el.parentElement;
  const chartInstance = window.Chart ? Chart.getChart(elementId) : null;
  ...
  try {
    ...
    // Forza la stessa larghezza "da desktop" un istante prima di catturare: senza
    // questo, su un telefono (dove il sito passa a 1 colonna) Chart.js avrebbe già
    // ridisegnato il grafico con proporzioni molto più larghe/schiacciate, e quella
    // forma sbagliata resterebbe "congelata" per sempre nell'immagine esportata.
    if (cardEl) {
      cardEl.style.width = LARGHEZZA_DESKTOP_CATTURA_CHART + 'px';
      if (chartInstance) chartInstance.resize();
    }
