...
/* ══════════════════════════════════════════════════════════════════════════
   PAGINA/SLIDE "ANALISI SEGMENTALE CORPOREA" — sagome del corpo con le linee di
   richiamo verso le zone misurate (circonferenze e plicometria), ciascuna con
   una mini-dashboard (card + sparkline) agganciata. Coordinate calibrate a mano
   insieme al dottore, sul prototipo di test, punto per punto sulla foto reale —
   NON modificare senza motivo: uno spostamento anche piccolo disallinea la
   linea dal punto anatomico giusto sul corpo.
   xPct/yPct = percentuale (0-100) rispetto a LARGHEZZA/ALTEZZA dell'immagine
   originale (quella con fronte+bilancia+retro). lato: 'sx' → card in colonna
   sinistra, 'dx' → colonna destra.
══════════════════════════════════════════════════════════════════════════ */
const PUNTI_ANATOMICI = {
  circonferenze_uomo: [
    { label: 'Collo',    campo: 'c_collo',            unit: 'cm', xPct: 22, yPct: 15, lato: 'sx' },
    { label: 'Torace',   campo: 'c_torace',           unit: 'cm', xPct: 19, yPct: 23, lato: 'sx' },
    { label: 'Vita',     campo: 'c_vita',             unit: 'cm', xPct: 21, yPct: 36, lato: 'sx' },
    { label: 'Fianchi',  campo: 'c_fianchi',          unit: 'cm', xPct: 83, yPct: 35, lato: 'dx' },
    { label: 'Braccio',  campo: 'c_braccio_contratto',unit: 'cm', xPct: 89, yPct: 25, lato: 'dx' },
    { label: 'Coscia',   campo: 'c_coscia',           unit: 'cm', xPct: 86, yPct: 53, lato: 'dx' }
  ],
  circonferenze_donna: [
    { label: 'Collo',    campo: 'c_collo',            unit: 'cm', xPct: 20, yPct: 17, lato: 'sx' },
    { label: 'Vita',     campo: 'c_vita',             unit: 'cm', xPct: 17, yPct: 33, lato: 'sx' },
    { label: 'Fianchi',  campo: 'c_fianchi',          unit: 'cm', xPct: 27, yPct: 37, lato: 'sx' },
    { label: 'Gluteo',   campo: 'c_gluteo',           unit: 'cm', xPct: 85, yPct: 42, lato: 'dx' },
    { label: 'Braccio',  campo: 'c_braccio_contratto',unit: 'cm', xPct: 90, yPct: 25, lato: 'dx' },
    { label: 'Coscia',   campo: 'c_coscia',           unit: 'cm', xPct: 87, yPct: 52, lato: 'dx' }
  ],
  plicometria_uomo: [
    { label: 'Pettorale',      campo: 'p_pettorale',      unit: 'mm', xPct: 18, yPct: 23, lato: 'sx' },
    { label: 'Ascellare',      campo: 'p_ascellare',      unit: 'mm', xPct: 16, yPct: 27, lato: 'sx' },
    { label: 'Addome',         campo: 'p_addome',         unit: 'mm', xPct: 25, yPct: 38, lato: 'sx' },
    { label: 'Soprailiaca',    campo: 'p_soprailiaca',    unit: 'mm', xPct: 18, yPct: 36, lato: 'sx' },
    { label: 'Coscia',         campo: 'p_coscia',         unit: 'mm', xPct: 17, yPct: 51, lato: 'sx' },
    { label: 'Tricipitale',    campo: 'p_tricipitale',    unit: 'mm', xPct: 89, yPct: 26, lato: 'dx' },
    { label: 'Sottoscapolare', campo: 'p_sottoscapolare', unit: 'mm', xPct: 84, yPct: 28, lato: 'dx' }
  ],
  plicometria_donna: [
    { label: 'Addome',         campo: 'p_addome',         unit: 'mm', xPct: 20, yPct: 37, lato: 'sx' },
    { label: 'Soprailiaca',    campo: 'p_soprailiaca',    unit: 'mm', xPct: 26, yPct: 36, lato: 'sx' },
    { label: 'Coscia',         campo: 'p_coscia',         unit: 'mm', xPct: 17, yPct: 50, lato: 'sx' },
    { label: 'Tricipitale',    campo: 'p_tricipitale',    unit: 'mm', xPct: 90, yPct: 25, lato: 'dx' },
    { label: 'Sottoscapolare', campo: 'p_sottoscapolare', unit: 'mm', xPct: 84, yPct: 26, lato: 'dx' }
  ]
};

const COLORE_CIRCONFERENZE_SAGOMA = COLORS_MINI_DASH.misure; // azzurro, fisso per tutte le circonferenze
const COLORE_PLICOMETRIA_SAGOMA = COLORS_MINI_DASH.pliche;   // ambra, fisso per tutte le pliche

// Dimensioni calibrate insieme al dottore sul prototipo di test (scala immagine del
// corpo, scala delle card, larghezza della colonna) — diverse tra uomo e donna perché
// le due foto hanno proporzioni leggermente diverse
const DIMENSIONI_SAGOMA = {
  M: { scalaImg: 1.09, scalaCard: 0.67, cardW_ig: 182 },
  F: { scalaImg: 1.10, scalaCard: 0.75, cardW_ig: 200 }
};
...
function renderingGraficiElite(visiteComplete, tentativoRiparazione = false) {
  if (!visiteComplete || visiteComplete.length === 0) return;

  const sesso = currentPatient.sesso;

  // Mostra SEMPRE il sesso attuale e nasconde l'altro, PRIMA di creare qualunque
  // grafico (non solo alla fine): se il contenitore di un grafico è ancora
  // display:none nel momento in cui Chart.js lo crea, Chart.js lo disegna con
  // dimensioni zero — bug reale trovato in produzione, capace di far sparire i
  // grafici sia a schermo sia nel PDF (che legge lo stesso canvas così com'è).
  // Prima si nascondeva solo l'altro sesso e non si "riaccendeva" mai più quello
  // attuale se era rimasto nascosto da un giro precedente.
  if (sesso === 'M') {
    document.querySelectorAll('[data-gender="donna"]').forEach(el => el.style.display = 'none');
    document.querySelectorAll('[data-gender="uomo"]').forEach(el => el.style.display = '');
  } else {
    document.querySelectorAll('[data-gender="uomo"]').forEach(el => el.style.display = 'none');
    document.querySelectorAll('[data-gender="donna"]').forEach(el => el.style.display = '');
  }
  ...
  // Pulizia difensiva: distrugge SEMPRE entrambe le versioni (uomo e donna) dei
  // grafici a doppio-sesso, prima di ricrearne solo una. Senza questo passaggio, se
  // si passa ad esempio da una paziente donna a un paziente uomo, i grafici versione
  // "donna" restano vivi in memoria (solo nascosti con display:none, mai distrutti),
  // perché il ramo "uomo" sotto non li tocca affatto. Su tante commutazioni di sesso
  // in una stessa sessione, queste istanze "fantasma" si accumulano e possono
  // interferire con i grafici mostrati — da qui i grafici mancanti dopo aver
  // consultato più pazienti senza mai ricaricare la pagina.
  ['chart-pliche-1-uomo', 'chart-pliche-2-uomo', 'chart-pliche-3-uomo', 'chart-pliche-4-uomo',
   ...
   'chart-circ-1-uomo', 'chart-circ-1-donna'
  ].forEach(idCanvas => {
    const istanzaEsistente = Chart.getChart(idCanvas);
    if (istanzaEsistente) istanzaEsistente.destroy();
  });
  ...
  // Auto-verifica (rete di sicurezza): se nonostante tutto un canvas del sesso attivo
  // risulta ancora con dimensioni zero (es. il browser non aveva ancora "misurato" il
  // contenitore nell'istante esatto in cui Chart.js lo ha creato), rigenera tutto
  // automaticamente UNA sola volta (tentativoRiparazione evita ogni rischio di loop),
  // senza che l'utente se ne accorga. Il piccolo ritardo (requestAnimationFrame) dà al
  // browser il tempo di applicare la visibilità appena impostata prima di ricontrollare.
  if (!tentativoRiparazione) {
    const idCanvasAttivi = sesso === 'M'
      ? ['chart-peso', 'chart-rmr-bmr', 'chart-composizione', 'chart-kg-massa', 'chart-somma-pliche',
         'chart-pliche-1-uomo', 'chart-pliche-2-uomo', 'chart-pliche-3-uomo', 'chart-pliche-4-uomo',
    ...
    const canvasRotto = idCanvasAttivi.some(id => {
      const el = document.getElementById(id);
      return el && (el.width === 0 || el.height === 0);
    });

    if (canvasRotto) {
      requestAnimationFrame(() => renderingGraficiElite(visiteComplete, true));
    }
  }
}

// Larghezza di riferimento "da desktop" per il CONTENITORE A GRIGLIA dei grafici
// (.dashboard-container max-width 1280px - padding 20px per lato). Su schermi stretti
// (telefono) il sito passa a 1 colonna per la visualizzazione normale, e Chart.js
// ridisegna ogni grafico con proporzioni molto diverse — queste funzioni forzano
// SEMPRE la stessa disposizione "da desktop" (2 colonne, piena larghezza) un istante
// prima di ogni cattura (PDF e Instagram), indipendentemente dal dispositivo con cui
// si genera l'esportazione. Si interviene sul CONTENITORE intero (non sulla singola
// card) apposta: la card "Somma Pliche" occupa il doppio delle altre (grid-column:
// 1/-1) — forzando solo la card si otteneva la sua larghezza dimezzata per errore,
// causando testo sgranato una volta reingrandita nel PDF/slide.
const LARGHEZZA_DESKTOP_GRIGLIA_GRAFICI = 1240;

// Forza temporaneamente la griglia di un grafico alla disposizione "da desktop" (2
// colonne, piena larghezza) e restituisce quanto serve per ripristinarla subito dopo
function forzaGrigliaDesktopPerCattura(canvasEl) {
  const gridEl = canvasEl ? canvasEl.closest('.charts-grid') : null;
  const chartInstance = (canvasEl && window.Chart) ? Chart.getChart(canvasEl.id) : null;
  if (!gridEl) return { gridEl: null, chartInstance, stileOriginale: null };

  const stileOriginale = {
    width: gridEl.style.width,
    gridTemplateColumns: gridEl.style.gridTemplateColumns
  };
  gridEl.style.width = LARGHEZZA_DESKTOP_GRIGLIA_GRAFICI + 'px';
  gridEl.style.gridTemplateColumns = 'repeat(2, 1fr)';
  if (chartInstance) chartInstance.resize();

  return { gridEl, chartInstance, stileOriginale };
}

// Ripristina la griglia alla sua disposizione normale (responsive, adatta al dispositivo)
function ripristinaGrigliaDopoCattura({ gridEl, chartInstance, stileOriginale }) {
  if (!gridEl) return;
  gridEl.style.width = stileOriginale.width;
  gridEl.style.gridTemplateColumns = stileOriginale.gridTemplateColumns;
  if (chartInstance) chartInstance.resize();
}

// Legge il contenuto attuale di un canvas Chart.js come immagine (usato dal PDF, che
// non fa uno screenshot ma legge direttamente il canvas), forzando prima la stessa
// disposizione "da desktop" e ripristinando tutto subito dopo — stessa identica logica
// già usata per le slide Instagram (vedi catturaCardInstagram), così il grafico ha
// sempre la forma corretta qualunque sia il dispositivo che genera il PDF.
...
async function generaGraficiProgressione(pdf) {
  try {
    ...
    const disegnaSerieMiniDashboard = (x, y, width, serieSingola, unit, decimals, colore) => {
      ...
      pdf.circle(x + 1.1, y + 1.4, 0.6, 'F');
      pdf.setFontSize(5.8);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(148, 163, 184);
      pdf.text(serieSingola.label.toUpperCase(), x + 2.6, y + 1.8, { maxWidth: Math.max(pillX - x - 2.6 - 1.5, 4) });

      // Valore finale, in grassetto, sopra la sparkline (come "21.0" nel riferimento)
      pdf.setFontSize(6.8);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(rgb.r, rgb.g, rgb.b);
      pdf.text(testoFinale, xDestro, y + 5.3, { align: 'right' });
    ...
    // Ogni serie ha la SUA propria card individuale — sfondo grigio chiarissimo neutro al
    // 50% di trasparenza + bordo grigio chiaro + angoli arrotondati — esattamente come le
    // ".mini-dash-item" a schermo sul sito, dove ogni metrica ha il proprio riquadro.
    const aggiuntaMiniDashboardBox = (x, y, width, dati) => {
      ...
      dati.serie.forEach((serieSingola, i) => {
        ...
        pdf.setGState(new window.jspdf.GState({ opacity: 0.5 }));
        pdf.setFillColor(248, 250, 252);
        pdf.roundedRect(xColonna - insetCard, y - insetCard, larghezzaColonna + insetCard * 2, miniDashHeight + insetCard * 2, raggioCard, raggioCard, 'F');
        pdf.setGState(new window.jspdf.GState({ opacity: 1 }));
        pdf.setDrawColor(226, 232, 240);
        pdf.setLineWidth(0.25);
        pdf.roundedRect(xColonna - insetCard, y - insetCard, larghezzaColonna + insetCard * 2, miniDashHeight + insetCard * 2, raggioCard, raggioCard, 'S');
    ...
    const gap = 6;
    const miniDashHeight = 19; // spazio riservato alla Mini-Dashboard (con sparkline) sotto ogni grafico
    ...
    for (let i = 0; i < page1Charts.length; i++) {
      ...
      aggiuntaMiniDashboardBox(gridX, gridY + miniChartHeight + 7, miniChartWidth, datiMiniDashboard[chartId]);
      ...
      if (i % 2 === 0) {
        ...
        gridX = marginL;
        gridY += miniChartHeight + miniDashHeight + gap + 7; // +7 per spazio tra righe (coerente col margine grafico-card sopra)
      }
    }
    ...
    for (let i = 0; i < plicheChartIds.length; i++) {
      ...
      if (i % 2 === 0) {
        ...
        gridX = marginL;
        gridY += miniChartHeight + miniDashHeight + gap + 7;
      }
    }
    ...
    // ═══════════════════════════════════════════════════════════════════
    // PAGINA 4 (NUOVA): ANALISI SEGMENTALE CORPOREA — sagome del corpo con le
    // linee di richiamo verso le zone misurate (Circonferenze + Plicometrie),
    // ciascuna con la propria mini-dashboard agganciata. Le pagine 1-3 sopra
    // restano identiche a prima: questa si aggiunge soltanto in fondo.
    // ═══════════════════════════════════════════════════════════════════

    pdf.addPage();
    await aggiungiWatermark(pdf);

    const dimSagoma = DIMENSIONI_SAGOMA[sesso] || DIMENSIONI_SAGOMA.M;
    const mmPerPxTest = 210 / 794; // conversione dalle unità del prototipo di test (tela 794px = pagina A4 da 210mm) alle unità reali del PDF
    const cardWSagomaPdf = dimSagoma.cardW_ig * mmPerPxTest;
    const immagineSagomaSrc = sesso === 'M' ? FILES.corpoUomo : FILES.corpoDonna;

    // Carica una volta l'immagine solo per conoscerne le proporzioni native (serve al
    // calcolo "contain": nessun taglio, nessuna deformazione, sempre centrata)
    const dimensioniImmagineSagoma = await new Promise(resolve => {
      const imgProbe = new Image();
      imgProbe.onload = () => resolve({ w: imgProbe.width, h: imgProbe.height });
      imgProbe.onerror = () => resolve({ w: 1, h: 1 });
      imgProbe.src = immagineSagomaSrc;
    });

    // Card di sfondo (bianco/grigio semi-trasparente + bordo + angoli arrotondati),
    // stesso identico stile già usato per le altre mini-dashboard del PDF. Margine
    // orizzontale e verticale separati apposta: con 5 card impilate (es. Plicometria
    // Uomo) lo spazio verticale disponibile per ciascuna è molto stretto — un margine
    // verticale come quello orizzontale (1.5mm) farebbe toccare/sovrapporre il bordo
    // con la card sopra/sotto. Il margine orizzontale resta invariato.
    const disegnaSfondoCardSagoma = (x, y, w, h) => {
      const raggioCard = 3, insetOrizzontale = 1.5, insetVerticale = 0.8;
      pdf.setGState(new window.jspdf.GState({ opacity: 0.5 }));
      pdf.setFillColor(248, 250, 252);
      pdf.roundedRect(x - insetOrizzontale, y - insetVerticale, w + insetOrizzontale * 2, h + insetVerticale * 2, raggioCard, raggioCard, 'F');
      pdf.setGState(new window.jspdf.GState({ opacity: 1 }));
      pdf.setDrawColor(226, 232, 240);
      pdf.setLineWidth(0.25);
      pdf.roundedRect(x - insetOrizzontale, y - insetVerticale, w + insetOrizzontale * 2, h + insetVerticale * 2, raggioCard, raggioCard, 'S');
    };

    // Intestazione di sezione con barretta colorata verticale — stile diverso dal
    // "sottolineato" usato nelle altre pagine, scelto di proposito solo per questa
    // pagina (confermato: va bene che si differenzi leggermente dal resto)
    const disegnaIntestazioneSagoma = (testo, y) => {
      const barW = 1.6, barH = 7;
      pdf.setFillColor(79, 70, 229);
      pdf.rect(marginL, y, barW, barH, 'F');
      pdf.setFontSize(13);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(15, 23, 42);
      pdf.text(testo.toUpperCase(), marginL + barW + 3, y + barH - 2);
      return barH;
    };

    // Disegna una sezione completa (Circonferenze o Plicometrie): intestazione,
    // immagine del corpo centrata, e per ogni punto anatomico la linea di richiamo
    // tratteggiata + la card con sparkline, impilate nella colonna sinistra o destra
    const disegnaSezioneSagoma = async (titolo, punti, colore, y, altezzaSezione) => {
      const barH = disegnaIntestazioneSagoma(titolo, y);
      const yContenuto = y + barH + 6;
      const altezzaContenuto = altezzaSezione - barH - 6;

      const gapCardImg = 5;
      const larghezzaImgDisponibile = (contentWidth - cardWSagomaPdf * 2 - gapCardImg * 2) * dimSagoma.scalaImg;
      const scalaFit = Math.min(larghezzaImgDisponibile / dimensioniImmagineSagoma.w, altezzaContenuto / dimensioniImmagineSagoma.h);
      const imgW = dimensioniImmagineSagoma.w * scalaFit;
      const imgH = dimensioniImmagineSagoma.h * scalaFit;
      const imgX = marginL + (contentWidth - imgW) / 2;
      const imgY = yContenuto + (altezzaContenuto - imgH) / 2;

      await aggiungiImmagineGlobal(pdf, immagineSagomaSrc, imgX, imgY, imgW, imgH);

      const rgb = hexToRgb(colore);
      const altezzaCard = 19; // stessa altezza già usata per le altre mini-dashboard (miniDashHeight)

      const puntiSx = punti.filter(p => p.lato === 'sx');
      const puntiDx = punti.filter(p => p.lato === 'dx');

      const disegnaColonna = (lista, colonnaX, allineaDestraCard) => {
        const n = lista.length;
        if (n === 0) return;
        const passo = altezzaContenuto / n;
        lista.forEach((p, i) => {
          const cardY = yContenuto + i * passo + (passo - altezzaCard) / 2;
          const serieSingola = { label: p.label, valori: visite.map(v => v[p.campo] || 0) };

          disegnaSfondoCardSagoma(colonnaX, cardY, cardWSagomaPdf, altezzaCard);
          disegnaSerieMiniDashboard(colonnaX, cardY, cardWSagomaPdf, serieSingola, p.unit, 1, colore);

          const puntoX = imgX + (p.xPct / 100) * imgW;
          const puntoY = imgY + (p.yPct / 100) * imgH;
          const ancoraX = allineaDestraCard ? colonnaX + cardWSagomaPdf : colonnaX;
          const ancoraY = cardY + altezzaCard / 2;

          pdf.setDrawColor(rgb.r, rgb.g, rgb.b);
          pdf.setLineWidth(0.3);
          pdf.setLineDashPattern([1, 1], 0);
          pdf.line(puntoX, puntoY, ancoraX, ancoraY);
          pdf.setLineDashPattern([], 0);

          pdf.setFillColor(rgb.r, rgb.g, rgb.b);
          pdf.circle(puntoX, puntoY, 1.3, 'F');
          pdf.setFillColor(255, 255, 255);
          pdf.circle(puntoX, puntoY, 0.6, 'F');
        });
      };

      disegnaColonna(puntiSx, marginL, true);
      disegnaColonna(puntiDx, marginL + contentWidth - cardWSagomaPdf, false);
    };

    // Barra blu del titolo, stesso stile della pagina 1 (voluta identica, non sottolineata)
    let y4 = marginT;
    const altezzaTitoloSagoma = 15;
    pdf.setFillColor(30, 58, 138);
    pdf.rect(0, y4, pageWidth, altezzaTitoloSagoma, 'F');
    pdf.setFontSize(16);
    pdf.setFont('helvetica', 'bold');
    pdf.setTextColor(255, 255, 255);
    pdf.text('ANALISI SEGMENTALE CORPOREA', pageWidth / 2, y4 + altezzaTitoloSagoma / 2 + 1.5, { align: 'center' });
    y4 += altezzaTitoloSagoma + 10;

    const gapSezioniSagoma = 8;
    const altezzaSezioneSagoma = (pageHeight - y4 - marginB - gapSezioniSagoma) / 2;

    const puntiCircSagoma = PUNTI_ANATOMICI['circonferenze_' + (sesso === 'M' ? 'uomo' : 'donna')];
    const puntiPlicaSagoma = PUNTI_ANATOMICI['plicometria_' + (sesso === 'M' ? 'uomo' : 'donna')];

    await disegnaSezioneSagoma('Circonferenze', puntiCircSagoma, COLORE_CIRCONFERENZE_SAGOMA, y4, altezzaSezioneSagoma);
    y4 += altezzaSezioneSagoma + gapSezioniSagoma;
    await disegnaSezioneSagoma('Plicometrie', puntiPlicaSagoma, COLORE_PLICOMETRIA_SAGOMA, y4, altezzaSezioneSagoma);
...
async function catturaCardInstagram(elementId, scale = 3) {
  ...
  try {
    ...
    // Forza la stessa disposizione "da desktop" un istante prima di catturare: senza
    // questo, su un telefono (dove il sito passa a 1 colonna) Chart.js avrebbe già
    // ridisegnato il grafico con proporzioni molto diverse, e quella forma sbagliata
    // resterebbe "congelata" per sempre nell'immagine esportata.
    const stato = forzaGrigliaDesktopPerCattura(el);
    try {
...
function disegnaSparklineIG(ctx, dati, x, y, w, h, colore, scala = 1) {
  if (dati.length === 0) return;
  if (dati.length === 1) {
    ctx.beginPath();
    ctx.fillStyle = colore;
    ctx.arc(x + w / 2, y + h / 2, 2.5 * scala, 0, Math.PI * 2);
    ctx.fill();
    return;
  }
...
function disegnaMiniDashboardIG(ctx, x, y, larghezza, serieList, unit, decimals, colore) {
  ...
  serieValide.forEach((s, i) => {
    ...
    // Misura PRIMA di disegnare se etichetta e pillola stanno fianco a fianco in questa
    // colonna, o se è troppo stretta (es. 3+ colonne nella stessa riga): in quel caso la
    // pillola scende sotto l'etichetta invece di restare a fianco, per non sovrapporsi.
    ctx.font = "700 13px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif";
    const testoEtichetta = s.label.toUpperCase();
    const largEtichetta = ctx.measureText(testoEtichetta).width;
    const testoDelta = `${freccia} ${segno}${delta.toFixed(decimals)}${unitaTxt}`;
    const largDelta = ctx.measureText(testoDelta).width + 20;
    const inizioEtichettaX = colX + padX + 14;
    const gapMinimoEtichettaPillola = 10;
    const cardStretta = (inizioEtichettaX + largEtichetta + gapMinimoEtichettaPillola + largDelta) > (colX + colW);

    const yEtichetta = cardStretta ? y + 14 : y + 22;

    ctx.beginPath();
    ctx.fillStyle = colore;
    ctx.arc(colX + padX + 4, yEtichetta, 4, 0, Math.PI * 2);
    ctx.fill();

    ctx.fillStyle = '#64748b';
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillText(testoEtichetta, inizioEtichettaX, yEtichetta);

    let sparkY, sparkH;
    if (cardStretta) {
      // Card stretta: pillola su una riga propria, sotto l'etichetta, allineata a
      // sinistra come lei (non più a destra: qui non deve più "incastrarsi" accanto
      // a nulla, ha la riga tutta per sé)
      const pillX = colX + padX;
      const pillY = y + 24;
      const pillH = 20;
      ctx.fillStyle = positivo ? 'rgba(225,29,72,0.12)' : (negativo ? 'rgba(5,150,105,0.12)' : 'rgba(100,116,139,0.12)');
      disegnaRettangoloArrotondatoIG(ctx, pillX, pillY, largDelta, pillH, 10);
      ctx.fill();
      ctx.fillStyle = coloreDelta;
      ctx.textAlign = 'center';
      ctx.fillText(testoDelta, pillX + largDelta / 2, pillY + pillH / 2);

      sparkY = y + 50;
      sparkH = 22;
    } else {
      const deltaX = colX + colW - padX - largDelta;
      ctx.fillStyle = positivo ? 'rgba(225,29,72,0.12)' : (negativo ? 'rgba(5,150,105,0.12)' : 'rgba(100,116,139,0.12)');
      disegnaRettangoloArrotondatoIG(ctx, deltaX, y + 9, largDelta, 24, 12);
      ctx.fill();
      ctx.fillStyle = coloreDelta;
      ctx.textAlign = 'center';
      ctx.fillText(testoDelta, deltaX + largDelta / 2, y + 22);
...
// Card singola ancorata a un punto anatomico (pagina/slide "Analisi Segmentale
// Corporea"): stessa identica ricetta visiva delle altre mini-dashboard (pallino,
// etichetta, pillola delta, sparkline, valori inizio/fine), ma con una scala libera
// (non fissa) per poterla dimensionare in base allo spazio scelto insieme al dottore
// sul prototipo di test. Ritorna l'altezza disegnata.
function disegnaCardMetricaAncorataIG(ctx, x, y, w, scala, label, valoriGrezzi, unit, decimals, colore) {
  const dati = (valoriGrezzi || []).filter(v => v !== null && v !== undefined && !isNaN(v));
  if (dati.length === 0) return 0;

  const iniziale = dati[0];
  const finale = dati[dati.length - 1];
  const delta = finale - iniziale;
  const direzioneInvertita = label === 'Massa Magra';
  const positivo = direzioneInvertita ? delta < 0 : delta > 0;
  const negativo = direzioneInvertita ? delta > 0 : delta < 0;
  const coloreDelta = positivo ? '#e11d48' : (negativo ? '#059669' : '#64748b');
  const freccia = delta > 0 ? '↑' : (delta < 0 ? '↓' : '→');
  ...
  ctx.globalAlpha = 0.5;
  ctx.fillStyle = 'rgb(248, 250, 252)';
  disegnaRettangoloArrotondatoIG(ctx, x, y, w, h, 12 * scala);
  ctx.fill();
  ctx.restore();
  ctx.strokeStyle = 'rgb(226, 232, 240)';
  ctx.lineWidth = 1.2 * scala;
  disegnaRettangoloArrotondatoIG(ctx, x, y, w, h, 12 * scala);
  ctx.stroke();

  const padX = 14 * scala;

  // Stessa logica adattiva della mini-dashboard standard: misura PRIMA se etichetta
  // e pillola stanno fianco a fianco, altrimenti la pillola scende sotto l'etichetta
  ctx.font = `700 ${13 * scala}px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif`;
  const testoEtichetta = label.toUpperCase();
  const largEtichetta = ctx.measureText(testoEtichetta).width;
  const testoDelta = `${freccia} ${segno}${delta.toFixed(decimals)}${unitaTxt}`;
  const largDelta = ctx.measureText(testoDelta).width + 20 * scala;
  const inizioEtichettaX = x + padX + 14 * scala;
  const gapMinimoEtichettaPillola = 10 * scala;
  const cardStretta = (inizioEtichettaX + largEtichetta + gapMinimoEtichettaPillola + largDelta) > (x + w);

  const yEtichetta = cardStretta ? y + 14 * scala : y + 22 * scala;

  ctx.beginPath();
  ctx.fillStyle = colore;
  ctx.arc(x + padX + 4 * scala, yEtichetta, 4 * scala, 0, Math.PI * 2);
  ctx.fill();
  ...
  if (cardStretta) {
    const pillX = x + padX;
    const pillY = y + 24 * scala;
    const pillH = 20 * scala;
    ctx.fillStyle = positivo ? 'rgba(225,29,72,0.12)' : (negativo ? 'rgba(5,150,105,0.12)' : 'rgba(100,116,139,0.12)');
    disegnaRettangoloArrotondatoIG(ctx, pillX, pillY, largDelta, pillH, 10 * scala);
    ctx.fill();
    ctx.fillStyle = coloreDelta;
    ctx.textAlign = 'center';
    ctx.fillText(testoDelta, pillX + largDelta / 2, pillY + pillH / 2);
    ...
    const deltaX = x + w - padX - largDelta;
    ctx.fillStyle = positivo ? 'rgba(225,29,72,0.12)' : (negativo ? 'rgba(5,150,105,0.12)' : 'rgba(100,116,139,0.12)');
    disegnaRettangoloArrotondatoIG(ctx, deltaX, y + 9 * scala, largDelta, 24 * scala, 12 * scala);
    ctx.fill();
    ctx.fillStyle = coloreDelta;
    ctx.textAlign = 'center';
    ctx.fillText(testoDelta, deltaX + largDelta / 2, y + 22 * scala);
  ...
  const sparkX = x + padX;
  const sparkW = w - padX * 2;
  disegnaSparklineIG(ctx, dati, sparkX, sparkY, sparkW, sparkH, colore, scala);

  ctx.font = `600 ${12 * scala}px -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif`;
  ctx.fillStyle = '#94a3b8';
  ctx.textAlign = 'left';
  ctx.fillText(`${iniziale.toFixed(decimals)}${unitaTxt}`, sparkX, sparkY + sparkH + 13 * scala);
  ctx.fillStyle = colore;
  ctx.textAlign = 'right';
  ctx.fillText(`${finale.toFixed(decimals)}${unitaTxt}`, sparkX + sparkW, sparkY + sparkH + 13 * scala);
...
// Su desktop: scarica subito la tela come file separato, esattamente come sempre —
// NESSUN cambiamento di comportamento. Su mobile (iPhone/Android): i download multipli
// ravvicinati non sono affidabili (la piattaforma tende a "tenere" solo l'ultimo), quindi
// invece di scaricarla la aggiunge a uno ZIP che verrà scaricato tutto insieme alla fine
// — stessa immagine, stessa risoluzione 2048×2048, stessa qualità PNG, nessuna perdita:
// cambia solo COME arriva sul telefono, non cosa contiene. Libera anche la memoria della
// tela subito dopo (solo su mobile, dove la pressione sulla memoria è il problema reale).
async function scaricaOAggiungiZip(canvas, nomeFile, isMobile, zip) {
  if (isMobile && zip) {
    const blob = await new Promise(resolve => canvas.toBlob(resolve, 'image/png'));
    if (blob) zip.file(nomeFile, blob);
    liberaCanvasIG(canvas);
  } else {
    await scaricaCanvasComePNG(canvas, nomeFile);
  }
}
...
async function scaricaSlideInstagram() {
  ...
  try {
    ...
    // Solo su mobile i 4 file verranno raggruppati in un unico .zip (vedi
    // scaricaOAggiungiZip): su desktop questa variabile non cambia nulla del
    // comportamento già esistente.
    const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
    const zip = isMobile && window.JSZip ? new JSZip() : null;

    const LATO = 1080;                    // spazio logico di disegno: tutte le posizioni/margini restano quelli già rifiniti
    const RISOLUZIONE_EXPORT = 2048;       // pixel reali del PNG finale: più alta = più nitida se si zooma sulla foto
    const MARGINE = 44;
    ...
    const nuovaTelaBase = () => {
      const canvas = document.createElement('canvas');
      canvas.width = RISOLUZIONE_EXPORT;
      canvas.height = RISOLUZIONE_EXPORT;
      const ctx = canvas.getContext('2d');
      // Tutto il codice sotto disegna usando coordinate 0-1080 (LATO): questa scala fa sì
      // che venga fisicamente renderizzato a piena risoluzione (2048px), senza dover
      // toccare nessun margine, dimensione o font già calibrato
      ctx.scale(RISOLUZIONE_EXPORT / LATO, RISOLUZIONE_EXPORT / LATO);
      ctx.fillStyle = '#ffffff';
    ...
    await scaricaOAggiungiZip(canvas1, `${nomePaziente} - Slide 1.png`, isMobile, zip);
    await attesaMs(400);
    ...
    await scaricaOAggiungiZip(canvas2, `${nomePaziente} - Slide 2.png`, isMobile, zip);
    await attesaMs(400);
    ...
    await scaricaOAggiungiZip(canvas3, `${nomePaziente} - Slide 3.png`, isMobile, zip);
    await attesaMs(400);

    // ─── SLIDE 4 (NUOVA): Analisi Segmentale Corporea — sagome del corpo con le
    // linee di richiamo verso le zone misurate (Circonferenze + Plicometrie),
    // ciascuna con la propria mini-dashboard agganciata. Le slide 1-3 sopra
    // restano identiche a prima: questa si aggiunge soltanto in fondo.
    const dimSagoma = DIMENSIONI_SAGOMA[sesso] || DIMENSIONI_SAGOMA.M;
    const immagineSagomaSrc = sesso === 'M' ? FILES.corpoUomo : FILES.corpoDonna;
    const imgSagoma = await new Promise(resolve => {
      const img = new Image();
      img.onload = () => resolve(img);
      img.onerror = () => resolve(null);
      img.src = immagineSagomaSrc;
    });

    const { canvas: canvas4, ctx: ctx4 } = nuovaTelaBase();
    y = MARGINE;
    await disegnaWatermarkIG(ctx4, LATO, { yTop: MARGINE + ALTEZZA_TITOLO + DISTACCO_WATERMARK });
    disegnaBarraTitoloIG(ctx4, 'ANALISI SEGMENTALE CORPOREA', MARGINE, y, CONTENUTO_W, ALTEZZA_TITOLO);
    y += ALTEZZA_TITOLO + GAP;

    const puntiCircSagoma = PUNTI_ANATOMICI['circonferenze_' + (sesso === 'M' ? 'uomo' : 'donna')];
    const puntiPlicaSagoma = PUNTI_ANATOMICI['plicometria_' + (sesso === 'M' ? 'uomo' : 'donna')];
    const gapSezioniSagomaIG = 30;
    const altezzaSezioneSagomaIG = (LATO - y - MARGINE - gapSezioniSagomaIG) / 2;

    // Disegna una sezione completa (Circonferenze o Plicometrie): intestazione,
    // immagine del corpo centrata, e per ogni punto anatomico la linea di richiamo
    // tratteggiata + la card con sparkline, impilate nella colonna sinistra o destra
    const disegnaSezioneSagomaIG = (titolo, punti, colore, ySezione, altezzaSezione) => {
      disegnaIntestazioneSezioneIG(ctx4, titolo, MARGINE, ySezione);
      const yContenuto = ySezione + 40 + GAP;
      const altezzaContenuto = altezzaSezione - 40 - GAP;
      if (!imgSagoma) return;

      const cardW = dimSagoma.cardW_ig;
      const gapCardImg = 18;
      const larghezzaImgDisponibile = (CONTENUTO_W - cardW * 2 - gapCardImg * 2) * dimSagoma.scalaImg;
      const scalaFit = Math.min(larghezzaImgDisponibile / imgSagoma.width, altezzaContenuto / imgSagoma.height);
      const imgW = imgSagoma.width * scalaFit;
      const imgH = imgSagoma.height * scalaFit;
      const imgX = MARGINE + (CONTENUTO_W - imgW) / 2;
      const imgY = yContenuto + (altezzaContenuto - imgH) / 2;

      ctx4.drawImage(imgSagoma, imgX, imgY, imgW, imgH);
      ...
      const disegnaColonna = (lista, colonnaX, allineaDestraCard) => {
        ...
        if (n === 0) return;
        const passo = altezzaContenuto / n;
        const cardH = 90 * dimSagoma.scalaCard;
        lista.forEach((p, i) => {
          const serieSingola = visite.map(v => v[p.campo] || 0);
          const cardY = yContenuto + i * passo + (passo - cardH) / 2;
          disegnaCardMetricaAncorataIG(ctx4, colonnaX, cardY, cardW, dimSagoma.scalaCard, p.label, serieSingola, p.unit, 1, colore);

          const puntoX = imgX + (p.xPct / 100) * imgW;
          const puntoY = imgY + (p.yPct / 100) * imgH;
          const ancoraX = allineaDestraCard ? colonnaX + cardW : colonnaX;
          const ancoraY = cardY + cardH / 2;

          ctx4.save();
          ctx4.setLineDash([4, 4]);
          ctx4.lineWidth = 1.4;
          ctx4.strokeStyle = colore;
          ctx4.beginPath();
          ctx4.moveTo(puntoX, puntoY);
          ctx4.lineTo(ancoraX, ancoraY);
          ctx4.stroke();
          ctx4.restore();

          ctx4.beginPath();
          ctx4.fillStyle = colore;
          ctx4.arc(puntoX, puntoY, 5, 0, Math.PI * 2);
          ctx4.fill();
          ctx4.beginPath();
          ctx4.fillStyle = '#ffffff';
          ctx4.arc(puntoX, puntoY, 2.2, 0, Math.PI * 2);
          ctx4.fill();
        });
      };

      disegnaColonna(puntiSx, MARGINE, true);
      disegnaColonna(puntiDx, MARGINE + CONTENUTO_W - cardW, false);
    };

    disegnaSezioneSagomaIG('Circonferenze', puntiCircSagoma, COLORE_CIRCONFERENZE_SAGOMA, y, altezzaSezioneSagomaIG);
    disegnaSezioneSagomaIG('Plicometrie', puntiPlicaSagoma, COLORE_PLICOMETRIA_SAGOMA, y + altezzaSezioneSagomaIG + gapSezioniSagomaIG, altezzaSezioneSagomaIG);

    await scaricaOAggiungiZip(canvas4, `${nomePaziente} - Slide 4.png`, isMobile, zip);

    // Solo su mobile: tutte e 4 le slide sono state raccolte nello zip (non ancora
    // scaricate) — le scarico ora tutte insieme, in un unico download affidabile
    if (isMobile && zip) {
      const contenutoZip = await zip.generateAsync({ type: 'blob' });
      const url = URL.createObjectURL(contenutoZip);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${nomePaziente} - Slide Instagram.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      setTimeout(() => URL.revokeObjectURL(url), 2000);
...
/* ══════════════════════════════════════════════════════════════════════════════
   BACKUP DATABASE — genera un unico file .sql con la struttura (CREATE TABLE) e
   tutti i dati (INSERT) di pazienti e visite, nell'ordine corretto (prima pazienti,
   poi visite, perché quest'ultima dipende dalla prima tramite paziente_id). Il file
   risultante può essere eseguito su QUALSIASI database Postgres (un altro progetto
   Supabase, un server proprio, un altro fornitore) per ricreare tutto identico —
   è pensato come "paracadute" per non dipendere da un unico fornitore nel tempo.
══════════════════════════════════════════════════════════════════════════════ */

// Struttura esatta delle due tabelle (nome colonna, tipo, se può essere vuota):
// presa direttamente da Supabase (information_schema), non indovinata
const COLONNE_BACKUP_PAZIENTI = [
  { nome: 'id', tipo: 'uuid', nullable: false },
  { nome: 'nominativo', tipo: 'text', nullable: false },
  { nome: 'sesso', tipo: 'char', nullable: false },
  { nome: 'data_nascita', tipo: 'date', nullable: true },
  { nome: 'created_at', tipo: 'timestamp', nullable: false }
];

const COLONNE_BACKUP_VISITE = [
  { nome: 'id', tipo: 'uuid', nullable: false },
  { nome: 'paziente_id', tipo: 'uuid', nullable: false },
  { nome: 'data_visita', tipo: 'date', nullable: false },
  { nome: 'eta', tipo: 'integer', nullable: false },
  { nome: 'peso', tipo: 'numeric', nullable: false },
  { nome: 'altezza', tipo: 'integer', nullable: false },
  { nome: 'laf', tipo: 'numeric', nullable: false },
  { nome: 'c_collo', tipo: 'numeric', nullable: true },
  { nome: 'c_torace', tipo: 'numeric', nullable: true },
  { nome: 'c_vita', tipo: 'numeric', nullable: true },
  { nome: 'c_fianchi', tipo: 'numeric', nullable: true },
  { nome: 'c_gluteo', tipo: 'numeric', nullable: true },
  { nome: 'c_braccio', tipo: 'numeric', nullable: true },
  { nome: 'c_coscia', tipo: 'numeric', nullable: true },
  { nome: 'p_pettorale', tipo: 'numeric', nullable: true },
  { nome: 'p_ascellare', tipo: 'numeric', nullable: true },
  { nome: 'p_addome', tipo: 'numeric', nullable: true },
  { nome: 'p_soprailiaca', tipo: 'numeric', nullable: true },
  { nome: 'p_tricipitale', tipo: 'numeric', nullable: true },
  { nome: 'p_sottoscapolare', tipo: 'numeric', nullable: true },
  { nome: 'p_coscia', tipo: 'numeric', nullable: true },
  { nome: 'created_at', tipo: 'timestamp', nullable: false },
  { nome: 'c_braccio_contratto', tipo: 'numeric', nullable: true }
];

function tipoSQLBackup(tipo) {
  switch (tipo) {
...
// I valori numerici non vanno mai tra apici (altrimenti Postgres li tratta come
// testo), tutti gli altri tipi (uuid, text, date, timestamp...) sì, con l'apice
// singolo "raddoppiato" per evitare di rompere la sintassi SQL se un nome contiene
// un apostrofo (es. "D'Angelo")
function formattaValoreBackup(valore, tipo) {
  if (valore === null || valore === undefined || valore === '') return 'NULL';
  if (tipo === 'integer' || tipo === 'numeric') return String(valore);
  return `'${String(valore).replace(/'/g, "''")}'`;
}

function costruisciCreateTableBackup(nomeTabella, colonne, vincoliExtra) {
  const righe = colonne.map(c => `  ${c.nome} ${tipoSQLBackup(c.tipo)}${c.nullable ? '' : ' NOT NULL'}`);
  return `CREATE TABLE IF NOT EXISTS ${nomeTabella} (\n${righe.join(',\n')}${vincoliExtra}\n);`;
}

// Righe inserite a blocchi da 500 (un solo INSERT gigante su migliaia di righe è
// più lento da eseguire e più rischioso da maneggiare come testo)
function costruisciInsertBackup(nomeTabella, colonne, righe) {
  if (!righe || righe.length === 0) return `-- Nessun dato presente in "${nomeTabella}" al momento del backup\n`;
  const nomiColonne = colonne.map(c => c.nome).join(', ');
  const dimensioneBlocco = 500;
  let sql = '';
  for (let i = 0; i < righe.length; i += dimensioneBlocco) {
    const blocco = righe.slice(i, i + dimensioneBlocco);
    const valoriBlocco = blocco
      .map(riga => '  (' + colonne.map(c => formattaValoreBackup(riga[c.nome], c.tipo)).join(', ') + ')')
      .join(',\n');
    sql += `INSERT INTO ${nomeTabella} (${nomiColonne}) VALUES\n${valoriBlocco};\n\n`;
  }
  return sql;
}

async function scaricaBackupDatabase() {
  const confermato = confirm(
    '⚠️ Stai per scaricare un backup completo del database: TUTTI i pazienti e TUTTE le visite registrate.\n\n' +
    'Il file generato (.sql) può essere usato per ripristinare tutto, identico, su qualsiasi altro database ' +
    'Postgres in caso di necessità.\n\nVuoi procedere?'
  );
  if (!confermato) return;

  const btn = document.getElementById('btn-backup-db');
  const testoOriginaleBtn = btn ? btn.innerHTML : '';
  if (btn) { btn.disabled = true; btn.innerHTML = '⏳ Backup...'; }

  try {
    const [{ data: pazienti, error: errorePazienti }, { data: visite, error: erroreVisite }] = await Promise.all([
      sbClient.from('pazienti').select('*'),
      sbClient.from('visite').select('*')
    ]);
    if (errorePazienti) throw errorePazienti;
    if (erroreVisite) throw erroreVisite;

    const adesso = new Date();
    const dataLeggibile = adesso.toLocaleString('it-IT');
    const dataFile = `${String(adesso.getDate()).padStart(2, '0')}-${String(adesso.getMonth() + 1).padStart(2, '0')}-${adesso.getFullYear()}`;

    let sql = '';
    sql += `-- ═══════════════════════════════════════════════════════════════\n`;
    sql += `-- BACKUP COMPLETO DATABASE — Calcolatore Nutrizionista\n`;
    sql += `-- Generato il: ${dataLeggibile}\n`;
    sql += `-- Contenuto: ${pazienti.length} pazienti, ${visite.length} visite\n`;
    sql += `--\n`;
    sql += `-- COME USARLO: eseguire l'intero contenuto di questo file su un database\n`;
    sql += `-- Postgres vuoto (es. un nuovo progetto Supabase, o un altro fornitore) per\n`;
    sql += `-- ricreare struttura e dati identici. L'ordine (prima pazienti, poi visite)\n`;
    sql += `-- è già quello corretto: non serve modificare nulla, va eseguito così com'è.\n`;
    sql += `-- ═══════════════════════════════════════════════════════════════\n\n`;

    sql += `-- ─────────── TABELLA: pazienti ───────────\n`;
    sql += costruisciCreateTableBackup('pazienti', COLONNE_BACKUP_PAZIENTI, ',\n  PRIMARY KEY (id)') + '\n\n';
    sql += costruisciInsertBackup('pazienti', COLONNE_BACKUP_PAZIENTI, pazienti);

    sql += `\n-- ─────────── TABELLA: visite ───────────\n`;
    sql += costruisciCreateTableBackup(
      'visite',
      COLONNE_BACKUP_VISITE,
      ',\n  PRIMARY KEY (id),\n  FOREIGN KEY (paziente_id) REFERENCES pazienti(id)'
    ) + '\n\n';
    sql += costruisciInsertBackup('visite', COLONNE_BACKUP_VISITE, visite);

    const blob = new Blob([sql], { type: 'application/sql' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `backup_calcolatore_${dataFile}.sql`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    setTimeout(() => URL.revokeObjectURL(url), 2000);
