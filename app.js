...
function renderMiniDashboardRow(containerId, serie, unit, decimals = 1, colore = '#4f46e5') {
  ...
  const blocchi = (serie || []).map(s => {
    ...
    const segno = delta > 0 ? '+' : '';
    const classe = delta > 0 ? 'positivo' : (delta < 0 ? 'negativo' : '');
...
async function generaGraficiProgressione(pdf) {
  try {
    ...
    const disegnaSerieMiniDashboard = (x, y, width, serieSingola, unit, decimals, colore) => {
      ...
      const coloreDelta = delta > 0 ? [225, 29, 72] : (delta < 0 ? [5, 150, 105] : [100, 116, 139]);
      const rgb = hexToRgb(colore);

      // Riga superiore: pallino + nome della serie a sinistra, pillola Δ a destra
      pdf.setFillColor(rgb.r, rgb.g, rgb.b);
      pdf.circle(x + 1, y + 1.1, 0.55, 'F');
      pdf.setFontSize(5.2);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(148, 163, 184);
      pdf.text(serieSingola.label.toUpperCase(), x + 2.4, y + 1.5, { maxWidth: width - 2.4 });

      pdf.setFontSize(6);
      pdf.setTextColor(coloreDelta[0], coloreDelta[1], coloreDelta[2]);
      pdf.text(testoDelta, xDestro, y + 1.5, { align: 'right' });
