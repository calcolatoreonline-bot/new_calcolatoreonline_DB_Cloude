...
const evidenziaUltimoPuntoPlugin = {
  ...
  afterDatasetsDraw(chart) {
    ...
    const margineSicurezza = 5;                 // respiro minimo tra due etichette (o tra etichetta e bordo) perché non si "tocchino"
    const altezzaTesto = 13;                    // altezza approssimativa di una riga di testo alla dimensione usata
    ...
    const etichette = punti.map(p => {
      ...
      // Continua a spostare l'etichetta (più in basso, poi più a destra) finché non è libera
      // da conflitti con ogni altra etichetta già piazzata sopra di lei — un vero controllo
      // di sovrapposizione, non una distanza fissa indovinata
      for (let tentativo = 0; tentativo < 12; tentativo++) {
        const box = rettangolo(xLinea, labelY, p.larghezzaTesto, allineamento);
        const conflitto = piazzate.find(q => sovrapposti(box, q.box));
        if (!conflitto) break;

        spostata = true;
        labelY = conflitto.box.bottom + margineSicurezza + altezzaTesto;
        xLinea = Math.max(xLinea, conflitto.box.right + margineSicurezza + 6);
        allineamento = 'left';
      }

      // Non uscire mai dal corridoio destro: se serve, ancora il testo al bordo invece di sforare
      let boxFinale = rettangolo(xLinea, labelY, p.larghezzaTesto, allineamento);
      if (boxFinale.right > limiteDestro) {
        xLinea = limiteDestro;
        allineamento = 'right';
        boxFinale = rettangolo(xLinea, labelY, p.larghezzaTesto, allineamento);
      }

      piazzate.push({ box: boxFinale });
      return { ...p, labelY, xLinea, allineamento, spostata };
    });
    ...
    etichette.forEach(e => {
      ...
      if (!e.spostata) {
        // Nessun conflitto: linea dritta verso l'alto dal puntino
        ctx.moveTo(e.x, e.y - 5);
        ctx.lineTo(e.x, e.labelY + 4);
      } else {
        // Conflitto rilevato: scarto a destra dal puntino, poi risalita fino all'etichetta
        ctx.moveTo(e.x, e.y);
...
function aggiornaVisitExtraDiff(visite, indexCorrente, visitaPrecedente) {
  ...
  const plicheRilevanti = PLICHE_CONFIG.filter(p => p.genders.includes(sesso));
...
function calcolaMassaGrassaPercentualeVisita(visita, sesso) {
  ...
  const somma = sesso === 'M'
    ...
    : (visita.p_addome||0)+(visita.p_soprailiaca||0)+(visita.p_tricipitale||0)+(visita.p_sottoscapolare||0)+(visita.p_coscia||0);
...
function generaTestoRiepilogoNarrativo(visiteComplete, sesso, nominativo) {
  ...
  const plicheFieldsFemmina = ['p_tricipitale', 'p_soprailiaca', 'p_addome', 'p_coscia', 'p_sottoscapolare'];
  ...
  const circFieldsMaschio = ['c_torace', 'c_vita', 'c_fianchi', 'c_braccio', 'c_coscia'];
  const circFieldsFemmina = ['c_vita', 'c_fianchi', 'c_gluteo', 'c_braccio', 'c_coscia'];
  const circFields = sesso === 'M' ? circFieldsMaschio : circFieldsFemmina;
  const nomiCirc = {
    c_torace: 'Torace', c_vita: 'Vita', c_fianchi: 'Fianchi',
    c_gluteo: 'Gluteo', c_braccio: 'Braccio', c_coscia: 'Coscia'
  };
  ...
  const distrettoPliche = trovaDistrettoPiuSignificativo(primaVisita, ultimaVisita, plicheFields, nomiPliche, SOGLIA_RIEPILOGO_PLICHE);
...
function renderingGraficiElite(visiteComplete) {
  ...
  const configComune = {
    ...
    layout: {
      ...
      // dell'ultimo punto, senza attraversare mai le curve del grafico
      padding: { right: 34 }
    },
    plugins: {
      legend: {
  ...
  renderMiniDashboardRow('mini-circ-braccio', [
    { label: 'Braccio', valori: visiteComplete.map(v => v.c_braccio || 0) },
...
async function salvaVisita() {
  if (!currentPatient) { alert('⚠️ Seleziona un paziente attivo.'); return; }
  if (currentVisitId) { alert('⚠️ Visita già registrata. Questa data ha già una visita salvata: usa "Elabora e Genera Report" per generarla, oppure seleziona nel calendario una data senza visita per registrarne una nuova.'); return; }
  const payload = {
  ...
  try {
    ...
    const datiMiniDashboard = {
      ...
      'chart-circ-braccio': {
        unit: 'cm', decimals: 1, colore: COLORS_MINI_DASH.misure,
        serie: [
          { label: 'Braccio', valori: visite.map(v => v.c_braccio || 0) },
